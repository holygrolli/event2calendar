...
public class MainHandlerTest {
	...
	public void testHandle() {
		...
		assertTrue(response.getBody().contains(Response.Status.URL_INVALID.toString()));
	}

	@Ignore
	public void testHandle2() throws UnsupportedEncodingException {
		...
		final AwsProxyResponse response = mainHandler.handleRequest(new AwsProxyRequestBuilder().path("/url/" + url).method(HttpMethod.GET).build(), new MockLambdaContext());
		assertNotNull("Repsonse must not be null", response);
		assertTrue(response.getBody().contains(Response.Status.OK.toString()));
	}
	@Ignore
	public void testHandle3() throws UnsupportedEncodingException {
	...
	@Ignore
	public void testHandleRequestMapping() throws UnsupportedEncodingException {
